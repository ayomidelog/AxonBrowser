...
pub async fn capture_screenshot() -> Result<Vec<u8>> {
    let page = current_page()?.ok_or_else(|| anyhow!("no Chrome page target exposed by DevTools"))?;
...
fn devtools_port() -> Result<Option<u16>> {
    ...
    Ok(Some(
        port.parse::<u16>()
            .with_context(|| format!("invalid DevTools port {:?}", port))?,
    ))
}
...
fn http_get_json(port: u16, path: &str) -> Result<String> {
    ...
    let response = String::from_utf8(response).context("Chrome DevTools HTTP response was not valid UTF-8")?;
    let (_, body) = response
        .split_once("\r\n\r\n")
...
async fn send_command(websocket_url: &str, method: &str, params: Value) -> Result<Value> {
    let (mut socket, _) = connect_async(websocket_url)
        .await
        .with_context(|| format!("failed to connect to Chrome DevTools websocket {}", websocket_url))?;
