...
pub async fn scroll_window(
    ...
    let browser_window = if scope.frame_selectors.is_empty() {
        crate::chrome::window::find_browser_window(None)?
    } else {
        let root = crate::chrome::page::root::resolve_page_scope(scope).await?;
        crate::chrome::actions::context::browser_window_for_node(&root).await?
    };
    let activation_note = crate::chrome::actions::context::activate_window_note(&browser_window.id);
