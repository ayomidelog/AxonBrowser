...
use crate::{chrome::retry, firefox::actions, inspect, model::LiveNode, window};
        ...
        let tabs = match resolve_tab_nodes(window_id).await {
            ...
            Err(_) => {
                let window = match window_id {
                    Some(id) => crate::firefox::window::find_firefox_window_by_id(id)?,
...
async fn resolve_tab_nodes(window_id: Option<&str>) -> Result<Vec<LiveNode>> {
    ...
    let fallback =
        crate::firefox::actions::click::resolve_chain_scoped(&["Page Tab List", "Page Tab"])
            .await?;
...
async fn resolve_locator_for_window(locator: &str, window_id: Option<&str>) -> Result<LiveNode> {
    if let Some(window_id) = window_id {
        let window = crate::firefox::window::find_firefox_window_by_id(window_id)?;
...
pub fn build_tab_infos(tabs: &[LiveNode], current: Option<&LiveNode>) -> Vec<TabInfo> {
    ...
    for tab in tabs.iter().cloned() {
        ...
        if lowered.contains("firefox privacy notice") {
            continue;
        }
    ...
    if !infos.iter().any(|tab| tab.is_current) && !infos.is_empty() {
