...
use crate::chrome::retry;
use crate::edge::{devtools, session};
...
pub async fn resolve_tabs_with_current() -> Result<Vec<TabInfo>> {
    retry::with_transient_retry(|| async {
        ...
        if tabs.is_empty() {
            return Err(anyhow!("no edge tabs matched"));
        }
...
pub async fn resolve_tabs_with_current_for_window(window_id: Option<&str>) -> Result<Vec<TabInfo>> {
    if let Some(window_id) = window_id {
        let _ = crate::edge::session::remember_browser_window_target(window_id);
    }
    resolve_tabs_with_current().await
}
...
mod tests {
    ...
    use crate::edge::devtools::PageInfo;
